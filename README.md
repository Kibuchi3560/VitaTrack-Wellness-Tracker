...
VitaTrack aims to provide a simple, offline, command-line tool that allows users to:

Record daily wellness metrics (water intake, sleep hours, exercise minutes, mood rating).

View a summary of their day's wellness status.

Receive instant, rule-based feedback on whether they met their daily wellness goals.

Build self-awareness and motivation through consistent tracking.

5. Proposed Features (Minimum 5)
#	Feature	Description
...
Water intake in glasses (integer)

Sleep hours (float)

Exercise minutes (integer)

Mood rating from 1 to 10 (integer)

Menu selection (integer)

7. Expected Output
A welcome message personalised with the user's name.

A formatted daily summary showing all logged values.

A calculated wellness score (0–100).

Pass/fail status for each wellness category.

Personalised feedback messages.

A final daily report card.